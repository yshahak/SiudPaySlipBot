...
A Telegram bot for Israeli employers of foreign caregivers (עובד זר בסיעוד). It runs a Hebrew-language FSM conversation, calculates monthly salary per Israeli labor law, generates a formal Hebrew PDF payslip, sends it, and immediately deletes all data. **Zero-Data Retention is a core product guarantee, not a nice-to-have.**
...
| File | Responsibility |
|---|---|
| `config.py` | All legal constants, dynamic wage lookup, paths |
...
Never add logging that persists personal data (names, passport numbers). Never buffer or cache `PayslipResult` objects.
...
- After any `bot.py` FSM change: trace the full conversation to confirm state transitions work for both full and partial months, with and without deductions

---

## Things to avoid

- Do not add persistence (database, file storage, logging of user data)
